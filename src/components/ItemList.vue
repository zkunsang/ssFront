<template>
  <v-container fluid>
    <v-layout text-center wrap justify-end column>
      <v-flex>
        <v-data-table
          dense
          :headers="headers"
        ...
        >
          ...
          <template v-slot:[`item.materialItem`]="{ item }">
            <div
              v-for="(materialItem, index) in itemArrangedMap[item.itemId]"
              :key="index"
            >
        ...
        <v-row>
          <v-col>
            ...
            <v-file-input
              accept=".csv"
              label="아이템 데이터(csv)"
              @change="importCSVItem"
            ></v-file-input>
          </v-col>
          <v-col>
            <v-btn @click="exportCSVItemExchange"
              >아이템 교환 데이터(csv)</v-btn
            >
            <v-file-input
              accept=".csv"
              label="아이템 교환 데이터(csv)"
              @change="importCSVItemExchange"
            ></v-file-input>
          </v-col>
        </v-row>
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    async onSave() {
      ...
      await saveFunc({
        itemInfo: this.itemEdit,
        materialItemList: this.materialItemList || [],
      });

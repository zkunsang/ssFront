      ...
      >
        ...
        <template v-slot:top>
          <v-toolbar flat color="white">
            <v-toolbar-title>tzAos</v-toolbar-title>
            <v-divider
              class="mx-4"
              inset
              vertical
...
export default {
  ...
  async created() {
    ...
    if(this.storyId) {
      console.log("created - ", this.storyId);
      await this.getList();
    }
  ...
  methods: {
    ...mapActions([
      'LIST_TZAOS_RESOURCE',
      'LIST_TZAOS_STORY_RESOURCE',
      'UPDATE_TZAOS_RESOURCE',
      'GET_TABLE_VERSION',
      'UPDATE_TABLE_VERSION',
      'DELETE_TZAOS_RESOURCE'
    ]),
    getDownloadUrl(item) {
      const {storyId, version, resourceId} = item;
      const url = `${getS3Url()}${storyId}/tzaos/${version}/${resourceId}`;
      return url;
    },
    async s3Uploads(list) {
      for(let i in list) {
        ...
        await s3Upload(item.file, `${this.storyId}/tzaos/${item.version}/${item.resourceId}`);
    ...
    async uploadFile() {
      ...
      await this.UPDATE_TZAOS_RESOURCE({
    ...
    async getList() {
      ...
      this.resourceList = await this.LIST_TZAOS_STORY_RESOURCE(this.storyId);
      console.log("- resourceList", this.resourceList);
    },
    async allResourceList() {
      return await this.LIST_TZAOS_RESOURCE();
    },
    ...
    async onDelete(item) {
      ...
      await this.DELETE_TZAOS_RESOURCE({ storyId, resourceId });
      await this.getList();
    }
  }
};

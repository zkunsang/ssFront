<template>
  <v-container>
    <v-layout text-center wrap>
      ...
      <v-container>
        <v-btn color="primary" dark class="mb-2" @click.prevent="createStory"
          >신규 스토리 등록</v-btn
        >
        <v-btn color="primary" dark class="mb-2" @click.prevent="exportCSVStory"
          >스토리 데이터 출력</v-btn
        >
      </v-container>
      <v-container>
      ...
      <v-card v-for="(story, index) in storyList" :key="index">
        <router-link :to="`/story/${story.storyId}/info`">
          <v-img :src="getSrcUrl(story)" width="212px" height="212px"> </v-img>
        </router-link>
        {{ story.storyId }}
      </v-card>
    </v-layout>
...
export default {
  ...
  methods: {
    ...
    getSrcUrl(storyData) {
      return storyData.thumbnail
        ? `${this.CDN_URL}${storyData.storyId}/thumbnail/${storyData.thumbnailVersion}/${storyData.thumbnail}`
        : no_image;
    },
    createStory() {
      this.dialog = true;

...
const actions = {
  LOGIN({ commit }, { adminId, password }) {
    return api.auth.login(adminId, password).then(({ sessionId, adminId }) => {
  ...
  LIST_TZAOS_STORY_RESOURCE(_, storyId) {
    return api.tzresource.list(storyId);
  },
  LIST_TZAOS_RESOURCE(_) {
    return api.tzresource.list();
  },
  UPDATE_TZAOS_RESOURCE(_, resourceList) {
    return api.tzresource.update(resourceList);
  },
  DELETE_TZAOS_RESOURCE(_, deleteInfo) {
    return api.tzresource.delete(deleteInfo);
  },
  LIST_DNN_RESOURCE(_) {

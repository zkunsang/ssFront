...
function readExcel(file, sheetReader) {
  return new Promise(async (resolve, reject) => {
    _readExcel(file, sheetReader, (data) => {
      resolve(data);
    });
  });
}

async function _readExcel(file, sheetReader, callback) {
  if (!file) return;
  ...
  reader.onload = (e) => {
    ...
    workbook.SheetNames.forEach(async (sheetName) => {
      var jsonObject = XLSX.utils.sheet_to_json(
        ...
        sheetReader[sheetName].range
      );
      retData[sheetName] = jsonObject;
    });
